...
def round_token(number, decimals, addr=''):
    if addr.lower() in {token.lower() for token in {WETH, CBETH, CBBTC, WSOL, PINTO_WETH_ADDR, PINTO_CBETH_ADDR}}:
        ...
        precision = 5
    elif addr.lower() in {token.lower() for token in {PINTO_CBBTC_ADDR}}:
        precision = 6
    else:
        precision = 0
...
def latest_pool_price_str(bean_client, addr):
    ...
    return f"{type_str}: deltaP [{round_num(delta_b, 0)}], price [${round_num(price, 4)}]"
...
def strip_custom_discord_emojis(text):
    ...
    stripped_type_0 = re.sub(r"<:[A-Za-z]+:[0-9]+>\s*", "", text)
    # :PU_PeepoPumpkin:
